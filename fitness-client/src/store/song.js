...
const song = {
  ...
  getters: {
    ...
    video: state => {
      ...
      if (!video) {
        video = JSON.parse(window.sessionStorage.getItem('video') || null)
      }
      return video
    },

    searchVideo: state => {
      let searchVideo = state.searchVideo
      if (!searchVideo) {
        searchVideo = JSON.parse(window.sessionStorage.getItem('searchVideo') || null)
      }
      return searchVideo
    }
  },
  mutations: {
    ...
    setSearchVideo: (state, searchVideo) => {
      state.searchVideo = searchVideo
      window.sessionStorage.setItem('searchVideo', JSON.stringify(searchVideo))
    }
  },
  actions: {}
}
